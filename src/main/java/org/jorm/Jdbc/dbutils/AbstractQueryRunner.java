...
public abstract class AbstractQueryRunner {
     ...
     *
     * @param stmt   PreparedStatement to fill
     * @param pmd    Prefetched parameter metadata
     * @param params Query replacement parameters; {@code null} is a valid
     *               value to pass in.
     * @throws SQLException if a database access error occurs
     */
     ...
     * is set to false.
     *
     * @param stmt PreparedStatement of which to query the metadata of parameters
    ...
    protected PreparedStatement prepareStatement(final Connection conn, final String sql)
        ...
        @SuppressWarnings("resource") final PreparedStatement ps = conn.prepareStatement(sql);
        try {
    ...
    protected PreparedStatement prepareStatement(final Connection conn, final String sql, final int returnedKeys)
        ...
        @SuppressWarnings("resource") final PreparedStatement ps = conn.prepareStatement(sql, returnedKeys);
        try {
            configureStatement(ps);
        } catch (final SQLException e) {
            ps.close();
